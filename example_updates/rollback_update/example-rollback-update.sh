...
function is_installed() {
    ...
    if ! [[ -f $2 ]]; then
        ...
        if [[ $grep_res -eq "0" ]]; then
            rc=900
            # Clear out the the updated flag and workflow since we have installed this update
            fw_setenv have_updated 0
            fw_setenv past_workflow ""
        else
            rc=901
        fi
        erc=0
...
InstallUpdate() {
    ...
    #
    have_updated=$(fw_printenv -n have_updated)
    past_workflow="$(fw_printenv -n past_workflow)"
    
    curr_workflow="$(basename $workfolder)"

    if [[ "$past_workflow" -eq "$curr_workflow" && $have_updated -eq 1 ]]; then
        # The current workflow is the same as the last workflow we did the update for
        # and we failed to install that update so we need to say "it failed"
        resultCode=0
        # ADUC_ERC_SWUPDATE_HANDLER_APPLY_FAILURE (0x30100300)
        extendedResultCode=0x30100300
        resultDetails="Image failed to be installed"
        ret_val=0
    else 
        # Clear out the the updated flag and workflow
        fw_setenv have_updated 0
        fw_setenv past_workflow ""
        ...
        if [[ $is_installed_ret -ne 0 ]]; then
            ...
            echo "Failed to determine whether the update has been installed or note."
        elif [[ $resultCode -eq 901 ]]; then
            # Not installed, need to check if we tried to install it already
...
ApplyUpdate() {
    ...
    # Set the updated flag and set the past_workflow to the current workflow
    fw_setenv have_updated 1
    fw_setenv past_workflow "$curr_workflow"
